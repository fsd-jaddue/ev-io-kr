  ...
  return (
    <>
      ...
      <article className="prose-ev max-w-3xl">
        ...
        <h2>제공하는 정보</h2>
        <ul>
          <li><Link href="/region">지역별 보조금</Link> — 17개 시·도와 시·군·구별 승용 전기차 지방비, 동일 모델 금액 확인 방법, 접수·출고·잔여 현황</li>
        ...
        <ul>
          <li>
            <strong>수치는 출처가 있는 값만</strong> — 지방비·국비·접수·출고·잔여 대수는 무공해차 통합누리집 공개 표를 수집한 값이나 지자체 공고문에서
            확인한 값만 싣습니다. 확인되지 않은 지역·차종은 임의 금액 대신 &ldquo;공고 확인&rdquo;으로 표시합니다.
          </li>
          <li>
            <strong>작성 방식과 근거를 구분합니다</strong> — 데이터 수집·정리와 초안 작성에 자동화 및 AI 도구를 활용합니다. 해설에는 확인한 공식 자료와 검토일을 표시하고, 수집값·계산 예시·해석을 구분합니다. 실제 상담이나 운행 경험을 수행한 것처럼 표현하지 않습니다.
          </li>
          <li>
            <strong>기준 시각을 함께 표시</strong> — 표마다 수집 시각과 출처 배지를 붙이고, 가이드에는 게시일·업데이트일을 적습니다. 값이 바뀌면
            표는 수집 성공 시 갱신합니다. 해설은 별도 검토가 필요하며 수집일을 글의 검토일로 대신하지 않습니다.
          </li>
          <li>
            <strong>정정</strong> — 오류 제보를 받으면 원 자료와 대조해 확인한 뒤 고치고, 해당 페이지의 업데이트일을 갱신합니다.
          </li>
        </ul>

        <h2>2026-09-20 주요 정정</h2>
        <p>전환지원 국비의 일괄 100만원 가산, 지역 최고액으로 차종 지방비를 역산하던 계산, 의무운행 2년·월할 환수 설명, 보조금 차감 후 금액을 취득세 과세표준으로 단정한 예시를 바로잡았습니다. 중복 구·군 문서는 시·도 표로 통합했습니다. <Link href="/guide/national-subsidy-calculation-2026">금액 계산 정정 근거</Link>와 <Link href="/guide/delivery-deadline-mandatory-period-refund">의무운행·환수 안내</Link>에서 확인할 수 있습니다.</p>
