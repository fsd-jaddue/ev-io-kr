...
const FAQ = [
  {
    ...
    a: "중·대형 승용 전기차 국비는 최대 580만원, 소형은 최대 530만원이며 내연기관차를 처분하고 구매하면 전환지원금 100만원이 추가됩니다. 여기에 거주 지역 지방비(서울 194만원부터 경북 울릉 1,224만원까지)가 더해집니다.",
  },
  {
    q: "지방비는 어디 기준으로 받나요?",
...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <section className="rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-800 px-6 py-10 text-white md:px-10 md:py-14">
      ...
      <section className="mt-12">
        ...
        <ol className="mt-4 grid gap-3 md:grid-cols-3">
          <Step n={1} title="국비 산정" body="성능보조금(주행거리·에너지효율)과 배터리안전보조금에 배터리효율·환경성·사후관리 계수를 곱하고, 차량가 5,300만원 미만은 100%, 8,500만원 미만은 50%를 적용합니다." />
          <Step n={2} title="지방비 비례" body="지자체 지방비는 국비 산정액에 비례해 지급됩니다. 국비를 100% 받는 차종은 지방비도 최대치를, 국비가 절반이면 지방비도 절반 수준을 받습니다." />
          <Step n={3} title="추가 인센티브" body="내연기관차 처분 시 전환지원금 100만원, 지자체별 다자녀·청년·취약계층 추가 지원, 취득세 최대 140만원 감면이 별도로 더해집니다." />
        </ol>
      </section>
        ...
        <ul className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {latestGuides.map((g) => (
            <li key={g.slug} className="rounded-xl border border-slate-200 p-4 transition hover:border-emerald-300">
              <p className="text-xs font-medium text-emerald-700">{g.category}</p>
              <Link href={`/guide/${g.slug}`} className="mt-1 block text-base font-bold text-slate-900 hover:underline">
                {g.title}
              </Link>
...
function Step({ n, title, body }: { n: number; title: string; body: string }) {
  return (
    <li className="rounded-xl border border-slate-200 p-4">
      <p className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-600 text-sm font-bold text-white">{n}</p>
