...
data/korea-map.ts    직접 그린 로우폴리 한국 지도(꼭짓점 테이블 + 시·도 폴리곤, 광역시 육각 박스, 제주 인셋) + 모바일 타일 배치
...
- **홈 히어로 = 잔여 현황 지도 보드**(2026-09-07). 배너 일러스트를 지도로 대체: PC는 `data/korea-map.ts`의 간략화 윤곽 지도(SVG), 모바일은 4열 타일 카토그램. 둘 다 렌더하고 CSS(`hidden md:block` / `md:hidden`)로 토글해 hydration 불일치를 피한다. 첫 페인트·SEO는 빌드 시 `getRemainSnapshot()`(동기 스냅샷, 라이브 시도 없음)으로 채우고, 마운트 후 `/api/remain`을 한 번 호출해 더 새로우면 교체한다. 지역 선택 → 아래 도킹 `RegionPanel`에서 시·군·구 타일(도) / 단일 공고 카드(특별·광역시·세종·제주) / `RemainTable embedded` 표로 드릴다운. 잔여 수준은 잔여/공고 기준 소진(≤0)·적음(<5%)·보통(5~15%)·여유(≥15%)·미수집 5단계이며 색은 마스크 재고 지도 관례(초록·노랑·주황·빨강). `RemainTable`은 `data`(재요청 생략)·`embedded`(표만) prop을 받는다.
